...
function defineGridNavigationTests(toolkitType: ToolkitType, keyMap: TestKeyMap) {
  test.describe(`${toolkitType} - ${keyMap.name} - basic grid navigation`, () => {
    ...
    test(`${toolkitType} - ${keyMap.name} - multiple keypress`, async ({ page }) => {
      await setCellFocus(page, { group: 1, row: 0, col: 0 });
      await page.keyboard.press(toKeyPressString(keyMap.map.right));
      await expectCellToContainActiveElement(page, { group: 1, row: 0, col: 1 });
      await page.keyboard.press(toKeyPressString(keyMap.map.right));
      await expectCellToContainActiveElement(page, { group: 1, row: 0, col: 2 });
      await page.keyboard.press(toKeyPressString(keyMap.map.right));
      await expectCellToContainActiveElement(page, { group: 1, row: 0, col: 3 });
      await page.keyboard.press(toKeyPressString(keyMap.map.down));
      await page.keyboard.press(toKeyPressString(keyMap.map.down));
      await page.keyboard.press(toKeyPressString(keyMap.map.down));
      await expectCellToContainActiveElement(page, { group: 1, row: 3, col: 3 });
      ...
      await expectCellToContainActiveElement(page, { group: 1, row: 9, col: 3 });
      ...
      await page.keyboard.press(toKeyPressString(keyMap.map.down));
      await page.keyboard.press(toKeyPressString(keyMap.map.down));
      await expectCellToContainActiveElement(page, { group: 1, row: 4, col: 1 });
      await page.keyboard.press(toKeyPressString(keyMap.map.up));
      await page.keyboard.press(toKeyPressString(keyMap.map.up));
      await page.keyboard.press(toKeyPressString(keyMap.map.up));
      await expectCellToContainActiveElement(page, { group: 1, row: 1, col: 1 });
    });
  });
}
